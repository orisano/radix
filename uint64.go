...
func (uint64Sorter) encode64(x uint64) uint64 {
	return x
}

func (s uint64Sorter) sort(a []uint64) {
	b := make([]uint64, len(a))
...
type uint64ParallelSorter struct {
	uint64Sorter
}

func (s uint64ParallelSorter) sort(a []uint64, p int) {
	b := make([]uint64, len(a))
