...
func SortInt32(a []int32) {
	...
	for _, v := range a {
		x := uint8((encodeInt32(v) >> 0) & 0xff)
		b[level0[x]] = v
		level0[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt32(v) >> 8) & 0xff)
		a[level1[x]] = v
		level1[x]++
	}
	for _, v := range a {
		x := uint8((encodeInt32(v) >> 16) & 0xff)
		b[level2[x]] = v
		level2[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt32(v) >> 24) & 0xff)
		a[level3[x]] = v
		level3[x]++
	}
}
...
func SortInt64(a []int64) {
	...
	for _, v := range a {
		x := uint8((encodeInt64(v) >> 0) & 0xff)
		b[level0[x]] = v
		level0[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt64(v) >> 8) & 0xff)
		a[level1[x]] = v
		level1[x]++
	}
	for _, v := range a {
		x := uint8((encodeInt64(v) >> 16) & 0xff)
		b[level2[x]] = v
		level2[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt64(v) >> 24) & 0xff)
		a[level3[x]] = v
		level3[x]++
	}
	for _, v := range a {
		x := uint8((encodeInt64(v) >> 32) & 0xff)
		b[level4[x]] = v
		level4[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt64(v) >> 40) & 0xff)
		a[level5[x]] = v
		level5[x]++
	}
	for _, v := range a {
		x := uint8((encodeInt64(v) >> 48) & 0xff)
		b[level6[x]] = v
		level6[x]++
	}
	for _, v := range b {
		x := uint8((encodeInt64(v) >> 56) & 0xff)
		a[level7[x]] = v
		level7[x]++
	}
}
...
func SortFloat32(a []float32) {
	...
	for _, v := range a {
		x := uint8((encodeFloat32(v) >> 0) & 0xff)
		b[level0[x]] = v
		level0[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat32(v) >> 8) & 0xff)
		a[level1[x]] = v
		level1[x]++
	}
	for _, v := range a {
		x := uint8((encodeFloat32(v) >> 16) & 0xff)
		b[level2[x]] = v
		level2[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat32(v) >> 24) & 0xff)
		a[level3[x]] = v
		level3[x]++
	}
}
...
func SortFloat64(a []float64) {
	...
	for _, v := range a {
		x := uint8((encodeFloat64(v) >> 0) & 0xff)
		b[level0[x]] = v
		level0[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat64(v) >> 8) & 0xff)
		a[level1[x]] = v
		level1[x]++
	}
	for _, v := range a {
		x := uint8((encodeFloat64(v) >> 16) & 0xff)
		b[level2[x]] = v
		level2[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat64(v) >> 24) & 0xff)
		a[level3[x]] = v
		level3[x]++
	}
	for _, v := range a {
		x := uint8((encodeFloat64(v) >> 32) & 0xff)
		b[level4[x]] = v
		level4[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat64(v) >> 40) & 0xff)
		a[level5[x]] = v
		level5[x]++
	}
	for _, v := range a {
		x := uint8((encodeFloat64(v) >> 48) & 0xff)
		b[level6[x]] = v
		level6[x]++
	}
	for _, v := range b {
		x := uint8((encodeFloat64(v) >> 56) & 0xff)
		a[level7[x]] = v
		level7[x]++
	}
}
