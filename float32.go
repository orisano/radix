...
func (float32Sorter) encode32(x float32) uint32 {
	v := math.Float32bits(x)
	return v ^ (((^v >> 31) - 1) | 0x80000000)
}

func (s float32Sorter) sort(a []float32) {
...
type float32ParallelSorter struct {
	float32Sorter
}

func (s float32ParallelSorter) sort(a []float32, p int) {
	b := make([]float32, len(a))
